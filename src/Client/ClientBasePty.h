...
class ClientBasePty :  public IHints<2, ClientBasePty>
{

public:
    using Arguments = std::vector<String>;

    explicit ClientBasePty(const std::string& tty_file_name_, int ttyFd, std::istream& iStream = std::cin, std::ostream& oStream = std::cout);
    ~ClientBasePty() override;

    void init();

    std::vector<String> getAllRegisteredNames() const override { return cmd_options; }

    Poco::Util::LayeredConfiguration& config() const { return app.config(); }
...
protected:
    ...
    std::string tty_file_name = "/dev/tty";
