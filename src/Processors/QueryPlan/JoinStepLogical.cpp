...
void JoinStepLogical::describeActions(FormatSettings & settings) const
{
    ...
    settings.out << prefix << "Left Expression:\n"
        << expression_actions.left_pre_join_actions.dumpDAG()
        ;
...
JoinPtr JoinStepLogical::chooseJoinAlgorithm(JoinActionRef & left_filter, JoinActionRef & right_filter, JoinActionRef & post_filter)
{
    ...
    if (join_info.kind == JoinKind::Paste)
        join_ptr = std::make_shared<PasteJoin>(table_join, input_headers[1]);
    else
        join_ptr = std::make_shared<HashJoin>(table_join, input_headers[1], settings[Setting::join_any_take_last_row]);
