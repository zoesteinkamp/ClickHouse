...
class ReleaseInfo:
    ...
    def update_version_and_contributors_list(self, dry_run: bool) -> None:
        ...
        with checkout(self.release_branch):
            ...
            cmd_commit_version_upd = f"{GIT_PREFIX} commit '{CMAKE_PATH}' '{CONTRIBUTORS_PATH}' -m 'Update autogenerated version and contributors'"
            cmd_push_branch = f"{GIT_PREFIX} push"
            Shell.check(
                cmd_commit_version_upd, dry_run=dry_run, verbose=True
            )
            Shell.check(cmd_push_branch, dry_run=dry_run, verbose=True)
            if dry_run:
                Shell.check(
